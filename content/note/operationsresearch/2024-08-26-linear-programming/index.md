...
- [线性规划对偶问题](#线性规划对偶问题)
    ...
    - [对偶单纯形](#对偶单纯形)
    - [对偶问题的应用](#对偶问题的应用)
- [拉格朗日乘子法和 KKT 条件](#拉格朗日乘子法和-kkt-条件)
    - [拉格朗日乘子法](#拉格朗日乘子法)
    - [无约束优化](#无约束优化)
    - [等式约束优化](#等式约束优化)
    - [不等式约束优化](#不等式约束优化)
        - [一元一次不等式约束问题](#一元一次不等式约束问题)
        - [多元多次不等式约束问题](#多元多次不等式约束问题)
...
为了方便起见通常将不等式统一为小于等于；
变量 `$x$` 在向量 `$l_{b}$` 到 `$u_{b}$`​ 之间取值；
目标函数的系数向量为 `$c$`，那么线性规划的标准形式如下所示：

`$$\text{minimize} f = c^{T}x$$`
`$$\begin{align}
...
上面是线性规划的 **标准形式**，但注意这种标准形式是针对程序设计工具而言的。
如果有凸优化理论的背景的人可能会感到疑惑：
为什么我看到的标准形式和你这里写的不太一样？是的，经典的凸优化教材会把模型写成另外一种形式：
...
为了和上面提出的标准形式区分，暂且把这种形式称作 **规范形式**。

规范形式求的是函数的极大值，并且把不等关系和等式关系统一为等式关系方便求解。
这时可能会有些疑惑，不等式怎么可以充当为方程呢？这就是一种数学思想。
有人可能可以理解方程是不等式的特例，但不一定理解不等式也可以视作方程的特例，
举个例子。比如对于不等式 `$2a+3b+c<10$`，左边比右边小，但是小多少呢？
把这个差额记作 `$d$`，左边如果补上这个差额就可以写作 `$2a+3b+c+d=10$`，
这样就转化成了等式。这里的 `$d$` 被称为松弛变量。
包括决策变量的上下界 `$l_{b}$` 和 `$u_{}$`  也会被转化为不等关系引入松弛量。
...
显然这不是线性规划数学模型的规范形式，在线性规划求解方法中，模型的规范形式如下：
...
* 如果变量 `$x_{k}$` 没有约束，则既可以是正数也可以是负数，
  令 `$x_{k} = x_{k}^{'} - x_{k}^{''}$`，其中 `$x_{k}^{'}x_{k}^{''} \geq 0$`。
...
将线性规划模型准换成规范型后，就可以使用经典的线性规划方法求解了，包括单纯形法、内点法、列生成法等。
...
去运输一批货物的时候大车能运 5 箱，小车只能运 3 箱，但我们大车和小车数量都有限，怎么安排运输方案能够在车辆够用的情况下运费还能最小？
如果我们把大车数量记为 `$x$`，小车数量记为 `$y$`，那么除了 `$x$` 和 `$y$` 的范围，`$5x+3y$` 也有自己的一个范围，算上运费作为优化目标。

### 生产原料问题

生产产品 A、B 需要原材料甲、乙、丙；生产一吨 A 需要多少多少甲、多少多少乙、多少多少丙，
这样就有了对 A，B 的三个原料约束。再来一个利润最大作为目标，这也是线性规划。
