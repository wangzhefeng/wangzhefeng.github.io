...
`$$f\big(x|\mu, \sigma^{2}\big) = \frac{1}{\sqrt{2 \sigma^{2} \pi}}e^{-\frac{(x-\mu)^{2}}{2\sigma^{2}}}$$`
...
`$$L(\mu, \sigma^{2}) = \prod_{i=1}^{n}f\big(x|\mu, \sigma^{2}\big)=\prod_{i=1}^{n}\frac{1}{\sqrt{2 \sigma^{2} \pi}}e^{\frac{(x_{i} - \mu)^{2}}{2 \sigma^{2}}}$$`
...
`$$f(x|\theta) = \sum_{k=1}^{K}\phi_{k}f\big(x|\mu_{k}, \sigma_{k}^{2}\big)=\sum_{k=1}^{K}\phi_{k}\frac{1}{\sqrt{2 \sigma^{2}_{k} \pi}} e^{-\frac{(x - \mu_{k})^{2}}{2 \sigma_{k}^{2}}}$$`
...
&= E_{\gamma}\big[l(\theta, \gamma|\theta^{(t)})\big] \\
&= \sum_{i=1}^{n}\sum_{k=1}^{K}E\big(\gamma_{ik} | x_{i}, \theta^{(t)}\big)\big[ln \phi_{k} + ln f(x | \mu_{k}, \sigma_{k}^{2})\big]
\end{align}$$`
...
E-step 通俗可以这样理解：固定住对参数 `$\theta$` 的估计，先更新对 `$\gamma$` 的估计（右脚不动，迈左脚）
...
`$$\sigma_{k}^{2(t+1)} = \frac{\sum_{i=1}^{n}\hat{\gamma}_{ik}\Big(x_{i} - \mu_{k}^{(t+1)}\Big)^{2}}{\sum_{i=1}^{n}\hat{\gamma}_{ik}}$$`
