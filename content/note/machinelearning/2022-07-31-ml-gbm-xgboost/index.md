...
-  对 cache
   优化不友好. 在预排序后, 特征对梯度的访问是一种随机访问, 并且不同的特征访问的顺序不一样, 无法对
   cache
   进行优化. 同时, 在每一层长树的时候, 需要随机访问一个行索引到叶子索引的数组, 并且不同特征访问的顺序也不一样, 也会造成较大的
   cache miss
...
:math:`L(\phi)=\sum_{i=1}^{n} l(y_i, \hat{y_i}) + \sum_{k=1}^{K} \Omega(f_k)`

其中: 

-  :math:`L(\cdot)`\ : 目标函数；

-  :math:`l(\cdot)`\ : 经验损失(误差)函数, 通常是凸函数. 用于刻画预测值
   :math:`\hat{y_i}` 和真实值
   :math:`y_i`\ 之间的差异, 即模型对训练数据的拟合程度

-  :math:`\Omega(\cdot)`\ : 模型的正则项. 用于降低模型的复杂度, 减轻过拟合问题
...
-  :math:`f_k`\ : 第 :math:`k` 棵树

-  :math:`y_i`\ : 目标变量

-  :math:`\hat{y_i}=\sum_{k=1}^{K}f_k(x_i), f_k \in F`
...
根据 Gradient Boosting 思想, 假设第 :math:`t` 轮迭代的目标函数: 

.. math:: L^{(t)} = \sum_{i=1}^{n} l \bigg(y_i, \hat{y_i}^{(t-1)} + f_t(x_i) \bigg) + \Omega(f_t) + constant

对目标函数 :math:`L^{(t)}` 在 :math:`\hat{y_i}^{(t-1)}`
...
.. math::
   ...
   -  函数\ :math:`f(x)`\ 在\ :math:`x_0`\ 处的二阶泰勒展开式: 
      :math:`f(x) = f(x_0) + f' (x_0)(x - x_0) + f''(x_0)(x - x_0)^2`

   -  目标函数 :math:`l(y_i, x)` 在 :math:`\hat{y_i}^{(t-1)}`
      处的二阶泰勒展开式: 
      :math:`l(y_i, x) = l(y_i, \hat{y_i}^{(t - 1)}) + \frac{\partial l(y_i, \hat{y_i}^{(t - 1)})}{\partial \hat{y_i}^{(t - 1)}} (x - \hat{y_i}^{(t - 1)}) + \frac{1}{2} \frac{{\partial ^2} l(y_i, \hat{y_i}^{(t - 1)}) } {\partial \hat{y_i}^{(t - 1)}} (x - \hat{y_i}^{(t - 1)})^2`

   -  令 :math:`x= \hat{y_i}^{(t-1)} + f_t (x_i)`, 记一阶导数为
      :math:`g_i = \frac{\partial l(y_i, \hat{y_i}^{(t - 1)})}{\partial \hat{y_i}^{(t - 1)}}`\ , 
      记二阶导数为
      :math:`h_i = \frac{{\partial ^2} l(y_i, \hat{y_i}^{(t - 1)})}{\partial \hat{y_i}^{(t - 1)}}`\ ；可以得到
      :math:`l(y_i, \hat{y}^{(t-1)} + f_t(x_i)) = l(y_i, \hat{y}^{(t - 1)}) + g_i f_t(x_i) + \frac{1}{2} h_i f_t^2 (x_i)`

上面的目标函数 :math:`L^{(t)}` 中, 第一项
:math:`l(y_i, \hat{y_i}^{(t-1)})` 以及 :math:`constant`
为常数项, 在优化问题中可直接删除: 

:math:`{\tilde L}^{(t)} = \sum_{i=1}^{n} [g_{i}f_{t}(x_i) + \frac{1}{2}h_{i}f_{t}^{2}(x_i)]  + \Omega (f_t)`

**模型正则项定义: **

通过\ ``叶子节点的权重得分``\ 和\ ``叶子节点的索引值(结构)``\ 定义一棵树: 

:math:`f_t(x) = \omega_{q(x)}, \omega \in R^T, q: R^d \rightarrow \{1, 2, \ldots, T\}`

其中: 

-  :math:`\omega`: 叶子节点权重得分向量

-  :math:`q(\cdot)`: 树结构

定义正则项(可以使其他形式): 

:math:`\Omega(f_t)=\gamma T + \frac{1}{2}\lambda\sum_{j=1}^{T}\omega_j^2`

其中: 

-  :math:`T` 是 叶节点的个数

-  :math:`\omega_j^2`\ 是叶子节点权重得分的L2范数

记决策树每个叶节点上的样本集合为: 

:math:`{I_j} = \{ {i | q(x_i) = j} \}`

将正则项 :math:`\Omega(f_t)` 展开: 
...
记 :math:`G_j=\sum_{i\in I_j}g_i`\ ,  :math:`H_j=\sum_{i\in I_j}h_i`
...
对于固定的树结构 :math:`q(x)`\ , 对 :math:`\omega_j`
求导等于0, 得到目标函数的解析解\ :math:`\omega_{j}^{\star}`\ : 

:math:`w_{j}^{\star} = \frac{\sum\limits_{i \in I_j} g_i}{\sum\limits_{i \in I_j} h_i + \lambda}`

即

:math:`w_{j}^{\star} = -\frac{G_j}{H_j+\lambda} `

将上面得到的解析解带入目标函数: 

:math:`{{\tilde L}^{( t )}}=-\frac{1}{2}\sum_{j=1}^{T}\frac{G_j^2}{H_j+\lambda} + \gamma T`
...
:math:`Gain = \frac{1}{2}\Bigg[\frac{G_{L}^{2}}{H_K + \lambda} + \frac{G_{R}^{2}}{H_R + \lambda} - \frac{(G_L + G_R)^{2}}{H_L + H_R + \lambda}\Bigg] - \gamma`

其中: 

-  :math:`\frac{G_{L}^{2}}{H_K + \lambda}`\ : 左子树分数；

-  :math:`\frac{G_{R}^{2}}{H_R + \lambda}`\ : 右子树分数；

-  :math:`\frac{(G_L + G_R)^{2}}{H_L + H_R + \lambda}`\ : 
   不分割可以得到的分数；

-  :math:`\gamma`\ : 假如新的子节点引入的复杂度代价；

对树的每次扩展, 都要枚举所有可能的分割方案；并且对于某次分割, 都要计算每个特征值左边和右边的一阶和二阶导数和, 从而计算这次分割的增益
:math:`Gain`\ ； 对于上面的分割增益
:math:`Gain`\ , 都要判断分割每次分割对应的 :math:`Gain`
的大小, 并且进行优化, 取最小的
:math:`Gain`\ , 直到当新引入一个分割带来的增益小于某个阈值时, 就去掉这个分割. 这里的优化, 相当于对树进行剪枝. 
...
Building XGBoost from source
   ...
   1. 首先从 C++ 代码构建共享库(libxgboost.so适用于Linux /
      OSX和xgboost.dllWindows)
...
1. 首先, 选择一个相对较大的\ ``learning rate``\ , 比如:0.1 (一般分为在:
   0.05-0.3). 根据这个选定的\ ``learning rate``\ 对树的数量\ ``number of tree``\ 进行CV调优；

2. 调节树参数: \ ``max_depth``, ``min_child_weight``, ``gamma``,
