...
我们经常需要的是<span style='border-bottom:1.5px dashed red;'>在这两个极端方法之间的方法</span>。
...
下面给出了两种等价形式的简单指数平滑，其中每一种都能推出预测方程式
...
* `$\alpha$` 是水平 `$0 \leq \alpha \leq 1$` 的平滑参数
* `$\beta^{*}$` 是趋势 `$0 \leq \beta^{*} \leq 1$` 的平滑参数
...
Holt 线性方法产生的预测值可以显示出来的趋势（增加或减少）。
...
除了平滑参数 `$\alpha$` 和 `$\beta^{*}$`（在 Holt 方法中，取值介于 0 和 1 之间），该方法还包含一个阻尼参数 `$0 < \phi < 1$`：
...
如果 `$\phi = 1$`，则该方法与霍尔特线性方法相同。对于 0 和 1 之间的取值，`$\phi$` 平缓了趋势，
...
`$\phi$` 的值接近 `$1$` 意味着包含阻尼系数额模型无法与不包含阻尼系数的模型区分开来。
