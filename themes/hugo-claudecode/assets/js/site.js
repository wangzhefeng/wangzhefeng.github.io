(() => {
  ...
  const messages = {
    ...
    en: {
      ...
      recentUpdatesDesc: "Recently updated and organized.",
  ...
  const updateUtterancesTheme = (theme = currentTheme()) => {
    ...
    const lightFallback = utterancesRoot.getAttribute("data-utterances-theme-light-fallback") || "github-light";
    const darkFallback = utterancesRoot.getAttribute("data-utterances-theme-dark-fallback") || "github-dark";
    ...
    const nextTheme = theme === "dark"
      ? resolveTheme(darkTheme, darkFallback)
      : resolveTheme(lightTheme, lightFallback);
    if (!nextTheme) return;
